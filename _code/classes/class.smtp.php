...
/**
 * PHPMailer RFC821 SMTP email transport class.
 * Implements RFC 821 SMTP commands and provides some utility methods for
...
class SMTP
{
  ...
  /**
   * Debug level to show connection status, client -> server and server ->
   * client messages
   */
   ...
   * represents the server name. In case of HELO it is the only element of the
   * array.
  ...
  public function connect($host, $port = null, $timeout = 30, $options = array())
  {
    ...
    if ($streamok)
    {
      ...
      $this->smtp_conn = @stream_socket_client($host . ":" . $port, $errno, 
          $errstr, $timeout, STREAM_CLIENT_CONNECT, $socket_context);
    } else
    {
      // Fall back to fsockopen which should work in more places, but is missing
      // some features
      $this->edebug(
   ...
   */
  public function authenticate($username, $password, $authtype = null, $realm = '', 
      $workstation = '')
  {
    if (! $this->server_caps)
    {
    ...
    if (array_key_exists('EHLO', $this->server_caps))
    {
      ...
      if (! array_key_exists('AUTH', $this->server_caps))
      {
        ...
        // 'at this stage' means that auth may be allowed after the stage
        // changes
        // e.g. after STARTTLS
        return false;
      }
      ...
      self::edebug(
          'Auth method requested: ' . ($authtype ? $authtype : 'UNKNOWN'), 
          self::DEBUG_LOWLEVEL);
      self::edebug(
          'Auth methods available on the server: ' .
               implode(',', $this->server_caps['AUTH']), self::DEBUG_LOWLEVEL);
    ...
    switch ($authtype)
    {
      case 'PLAIN':
        ...
        if (! $this->sendCommand('User & Password', 
            base64_encode("\0" . $username . "\0" . $password), 235))
        {
          return false;
        }
        break;
      ...
      case 'NTLM':
        ...
        if (! $this->sendCommand('AUTH NTLM', 
            'AUTH NTLM ' . base64_encode($msg1), 334))
        {
          return false;
        }
        ...
        $ntlm_res = $ntlm_client->NTLMResponse(substr($challenge, 24, 8), 
            $password);
        // msg3
        $msg3 = $ntlm_client->TypeMsg3($ntlm_res, $username, $realm, 
            $workstation);
  ...
  public function data($msg_data)
  {
     ...
     * According to rfc821 we should not send more than 1000 characters on a
     * single line (including the CRLF)
     * so we will break the data up into lines by \r and/or \n then if needed we
     * will break each of those into
     * smaller lines to fit within the limit.
     * We will also look for lines that start with a '.' and prepend an
     * additional '.'.
    ...
    /*
     * To distinguish between a complete RFC822 message and a plain message
     * body, we check if the first field
     * of the first line (':' separated) does not contain a space then it
     * _should_ be a header and we will
    ...
    foreach ($lines as $line)
    {
      ...
      // Micro-optimisation: isset($str[$len]) is faster than (strlen($str) >
      // $len),
      while (isset($line[self::MAX_LINE_LENGTH]))
      {
        // Working backwards, try to find a space within the last
        // MAX_LINE_LENGTH chars of the line to break on
        ...
        // If processing headers add a LWSP-char to the front of new line RFC822
        // section 3.1.1
        if ($in_headers)
        {
          $line = "\t" . $line;
  ...
  public function hello($host = '')
  {
    ...
    return (boolean) ($this->sendHello('EHLO', $host) or
   ...
   * In case of HELO, the only parameter that can be discovered is a server
   * name.
   * 
   * @access protected
   * @param string $type
   ...
   * Closes the socket if there is no error or the $close_on_error argument is
   * true.
  ...
  {
    return $this->sendCommand('RCPT TO', 'RCPT TO:<' . $toaddr . '>', 
        array(
            250,
            251
        ));
  ...
  protected function sendCommand($command, $commandstring, $expect)
  {
    ...
    $matches = array();
    if (preg_match("/^([0-9]{3})[ -](?:([0-9]\\.[0-9]\\.[0-9]) )?/", 
        $this->last_reply, $matches))
    {
      $code = $matches[1];
   ...
   *          The data to send
   * @access public
   * @return integer|boolean The number of bytes sent to the server or false on
   *         error
   */
   ...
   * The method works in three ways, dependent on argument value and current
   * state
   * 1.
  ...
  protected function get_lines()
  {
    ...
    while (is_resource($this->smtp_conn) && ! feof($this->smtp_conn))
    {
      ...
      // micro-optimisation over strlen
   ...
   *          The name of the mechanism to use for debugging output, or a
   *          callable to handle it.
   */
