...
class PHPMailer
{
   ...
   *             it's the receiver's job (RFC5321 section 4.4), so this no
   ...
   * Set to 0 to not wrap. A useful value here is 78, for RFC2822 section 2.1.1
   * compliance.
   * @type integer
   */
  public $WordWrap = 0;
   ...
   * Be aware that in PHP >= 5.6 this requires that the server's certificates
   * are valid.
   * @type boolean
   */
   ...
   * Options: An empty string for PHPMailer default, whitespace for none, or a
   * string to use
   * @type string
   */
  public $XMailer = '';
   ...
   *          * `php` Use PHP built-in FILTER_VALIDATE_EMAIL; same as pcre8 but
   *          does not allow 'dotless' domains;
   *          * `html5` Use the pattern given by the HTML5 spec for 'email' type
   *          form input elements.
  ...
  public static function validateAddress($address, $patternselect = 'auto')
  {
    if (! $patternselect or $patternselect == 'auto')
    {
      ...
      {
        // Filter_var appeared in PHP 5.2.0 and does not require the PCRE
        // extension
    ...
    switch ($patternselect)
    {
      case 'pcre8':
        /**
         * Uses the same RFC5322 regex on which FILTER_VALIDATE_EMAIL is based,
         * but allows dotless domains.
         * 
      ...
      case 'html5':
        /**
         * This is the pattern used in the HTML5 spec for validation of 'email'
      ...
      case 'noregex':
        ...
        // Check the address is 3 chars or longer and contains an @ that's not
   ...
   * 
   * @throws phpmailerException
   * @return boolean false on error - See the ErrorInfo property for details of
   *         the error.
   */
  ...
  public function preSend()
  {
    try
    {
      ...
      // Create body before headers in case body makes changes to headers (e.g.
      ...
      if (! empty($this->DKIM_domain) && ! empty($this->DKIM_private) &&
           ! empty($this->DKIM_selector) && file_exists($this->DKIM_private))
      {
  ...
  protected function sendmailSend($header, $body)
  {
    ...
    } else
    {
      ...
      fputs($mail, $body);
      $result = pclose($mail);
      $this->doCallback(($result == 0), $this->to, $this->cc, $this->bcc, 
  ...
  protected function mailSend($header, $body)
  {
    ...
    if ($this->SingleTo && count($toArr) > 1)
    {
      ...
      {
        $result = $this->mailPassthru($toAddr, $this->Subject, $body, $header, 
            $params);
  ...
  protected function smtpSend($header, $body)
  {
    ...
    {
      $this->setError(
          $this->lang('from_failed') . $smtp_from . ' : ' .
    ...
    if (count($bad_rcpt) > 0)
    {
      ...
      throw new phpmailerException($this->lang('recipients_failed') . $errstr, 
          self::STOP_CONTINUE);
    }
    return true;
  }
  ...
  public function smtpConnect($options = array())
  {
    ...
    foreach ($hosts as $hostentry)
    {
      ...
      if ('ssl' == $hostinfo[2] or
           ('' == $hostinfo[2] and 'ssl' == $this->SMTPSecure))
      {
        $prefix = 'ssl://';
      ...
      {
        // Check for an OpenSSL constant rather than using extension_loaded,
        // which is sometimes disabled
        if (! $sslext)
        {
          throw new phpmailerException(
              $this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
        }
      }
      $host = $hostinfo[3];
      $port = $this->Port;
      ...
      if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options))
      {
        try
        {
          ...
          if ($this->SMTPAutoTLS and $sslext and $secure != 'ssl' and
   ...
   * break them.
   * This is called automatically by createBody(), so you don't need to call it
   * yourself.
   * 
   * @access public
   * @return void
  ...
  public function createHeader()
  {
    ...
    if (($this->Mailer == 'sendmail' or $this->Mailer == 'qmail' or
         $this->Mailer == 'mail') and count($this->bcc) > 0)
    {
    ...
    {
      $this->lastMessageID = sprintf('<%s@%s>', $this->uniqueid, 
          $this->ServerHostname());
    }
  ...
  public function getMailMIME()
  {
    ...
    {
      // RFC 2045 section 6.4 says multipart MIME parts may only use 7bit, 8bit
      // or binary CTE
      if ($ismultipart)
      {
        ...
        // The only remaining alternatives are quoted-printable and base64,
      // which are both 7bit compatible
      } else
      {
  ...
  public function createBody()
  {
    ...
    // If lines are too long, and we're not already using an encoding that will
    // shorten them,
    ...
    switch ($this->message_type)
    {
      ...
      case 'alt':
        $body .= $mimepre;
        $body .= $this->getBoundary($this->boundary[1], $altBodyCharSet, 
      ...
      case 'alt_attach':
        ...
        $body .= $this->textLine("\tboundary=\"" . $this->boundary[2] . '"');
        $body .= $this->LE;
        $body .= $this->getBoundary($this->boundary[2], $altBodyCharSet, 
            'text/plain', $altBodyEncoding);
        $body .= $this->encodeString($this->AltBody, $altBodyEncoding);
        $body .= $this->LE . $this->LE;
        $body .= $this->getBoundary($this->boundary[2], $bodyCharSet, 
    ...
    } elseif ($this->sign_key_file)
    {
      try
      {
        ...
        // @TODO would be nice to use php://temp streams here, but need to wrap
        // for PHP < 5.1
        $file = tempnam(sys_get_temp_dir(), 'mail');
        if (false === file_put_contents($file, $body))
        {
          throw new phpmailerException(
              $this->lang('signing') . ' Could not write temp file');
        }
        $signed = tempnam(sys_get_temp_dir(), 'signed');
        // Workaround for PHP bug https://bugs.php.net/bug.php?id=69197
        if (empty($this->sign_extracerts_file))
        {
          $sign = @openssl_pkcs7_sign($file, $signed, 
        ...
        if ($sign)
        {
          ...
          @unlink($signed);
          // The message returned by openssl contains both headers and body, so
        ...
        if (! (empty($disposition)))
        {
          ...
          if (preg_match('/[ \(\)<>@,;:\\"\/\[\]\?=]/', $encoded_name))
          {
            $mime[] = sprintf('Content-Disposition: %s; filename="%s"%s', 
                $disposition, $encoded_name, $this->LE . $this->LE);
          } else
          {
            $mime[] = sprintf('Content-Disposition: %s; filename=%s%s', 
                $disposition, $encoded_name, $this->LE . $this->LE);
  ...
  public function encodeHeader($str, $position = 'text')
  {
    ...
    switch (strtolower($position))
    {
      ...
      case 'text':
      default:
        $matchcount += preg_match_all('/[\000-\010\013\014\016-\037\177-\377]/', 
            $str, $matches);
        break;
    }
    ...
    {
      // More than a third of the content will need encoding, so B encoding will
      // be most efficient
      $encoding = 'B';
  ...
  public function hasMultiBytes($str)
  {
    ...
    { // Assume no multibytes (we can't handle without mbstring functions
             // anyway)
      return false;
    }
  }
  ...
  /**
   * Backward compatibility wrapper for an old QP encoding function that was
   * removed.
   * 
  ...
  public function encodeQ($str, $position = 'text')
  {
    ...
    if (preg_match_all("/[{$pattern}]/", $encoded, $matches))
    {
      ...
      {
        $encoded = str_replace($char, '=' . sprintf('%02X', ord($char)), 
            $encoded);
      }
    }
   ...
   * @return boolean True on successfully adding an attachment
   */
  public function addStringEmbeddedImage($string, $cid, $name = '', 
      $encoding = 'base64', $type = '', $disposition = 'inline')
  {
  ...
  protected function serverHostname()
  {
    ...
    } elseif (isset($_SERVER) and array_key_exists('SERVER_NAME', $_SERVER) and
  ...
  public function msgHTML($message, $basedir = '', $advanced = false)
  {
    ...
    if (isset($images[2]))
    {
      foreach ($images[2] as $imgindex => $url)
      {
        ...
        if (preg_match('#^data:(image[^;,]*)(;base64)?,#', $url, $match))
        {
          ...
          if ($this->addStringEmbeddedImage($data, $cid, '', 'base64', 
              $match[1]))
          {
        ...
        } elseif (! preg_match('#^[A-z]+://#', $url))
        {
          ...
          if ($this->addEmbeddedImage($basedir . $directory . $filename, $cid, 
              $filename, 'base64', 
              self::_mime_types(
                  (string) self::mb_pathinfo($filename, PATHINFO_EXTENSION))))
          {
            $message = preg_replace(
                '/' . $images[1][$imgindex] . '=["\']' . preg_quote($url, '/') .
                     '["\']/Ui', $images[1][$imgindex] . '="cid:' . $cid . '"', 
                    $message);
          }
        }
      }
    ...
    $this->AltBody = $this->normalizeBreaks(
  ...
  public function html2text($html, $advanced = false)
  {
    ...
    return html_entity_decode(
        trim(
            strip_tags(
                preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\\1>/si', 
   ...
   * Drop-in replacement for pathinfo(), but multibyte-safe,
   * cross-platform-safe, old-version-safe.
   ...
   *          or a string name to return only the specified piece, allows
   *          'filename' to work on PHP < 5.2
   * @return string|array
   * @static
   *
   */
   ...
   * You should avoid this function - it's more verbose, less efficient, more
   * error-prone and
   ...
   * Converts UNIX LF, Mac CR and Windows CRLF line breaks into a single line
   * break format.
  ...
  public function DKIM_HeaderC($signHeader)
  {
    ...
    foreach ($lines as $key => $line)
    {
      ...
      $lines[$key] = $heading . ':' . trim($value); // Don't forget to remove
                                                    // WSP around the value
    }
    $signHeader = implode("\r\n", $lines);
    return $signHeader;
  }
  ...
  public function DKIM_Add($headers_line, $subject, $body)
  {
    ...
    $DKIMtime = time(); // Signature Timestamp = seconds since 00:00:00 - Jan 1,
                        // 1970 (UTC time zone)
    ...
    $dkimhdrs = 'DKIM-Signature: v=1; a=' . $DKIMsignatureType . '; q=' .
         $DKIMquery . '; l=' . $DKIMlen . '; s=' . $this->DKIM_selector . ";\r\n" .
  ...
  public static function hasLineLongerThanMax($str)
  {
    ...
    return (boolean) preg_match('/^(.{' . (self::MAX_LINE_LENGTH + 2) . ',})/m', 
        $str);
  }
